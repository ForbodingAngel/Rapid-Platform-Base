...
class simple_customize
{
    ...
    function load_i18n() {
        ...
        load_plugin_textdomain( 'simple-customize-plugin', false, $i18n_dir );
    }
	...
	function custom_post_type() {
		register_post_type(
			...
			array(
				...
				'can_export'          => false,
				'labels'              => array(
					'name' => __( 'Simple Customize Debug', 'simple-customize-plugin' )
				)
			)
		);
		...
		register_post_type(
			...
			array(
				...
				'can_export'          => false,
				'labels'              => array(
					'name' => __( 'Simple Customize Fonts Debug', 'simple-customize-plugin' )
				)
			)
		);
		...
		register_taxonomy(
			...
			array(
				...
				'show_ui' => $this->debug,
				'labels'  => array(
					'name' => __( 'Categories', 'simple-customize-plugin' )
				)
			)
		);
	}
    ...
    function generate_css( $force = false, $theme = null )
    {
	    ...
	    if ( $force || false === ( $css = get_transient( 'simple-customize-css' ) ) ) {
		    ...
		    foreach ( $entries AS $entry ) {
			    ...
			    if ( true === $settings['advanced'] || false === $settings['minified'] ) {
				    $css .= "\n";
			    }
		    }
	    }
